...
public class ImageProcessing {
    ...
    public static void main(String[] args) {
        ...
        // trim the border of the img then save it
        ...
        twoDToImage(negativeImage, "./negative_colored_apple.jpg");

        int[][] stretchedHorizontalImage = stretchHorizontally(imageData);
        twoDToImage(stretchedHorizontalImage, "./horizontal_stretched_apple.jpg");
        ...
        int[][] filtedImage = colorFilter(imageData, 30, -15 , 35);
        twoDToImage(filtedImage, "./filted_apple.jpg");

        int[][] canvas = new int[25][25];
        int[][] newCanvas = paintRandomImage(canvas);
        twoDToImage(newCanvas, "./random_painting.jpg");

        int[] rgba = {80, 55, 10, 255};
        int color = getColorIntValFromRGBA(rgba);
        int[][] canvasWithRectangle = paintRectangle(imageData, 50 , 30, 70, 90, color);
        twoDToImage(canvasWithRectangle, "./canvas_with_rectangle.jpg");

        imageData = imgToTwoD("./kitten.jpg");
        newCanvas = generateRectangles(imageData, 4);
        twoDToImage(newCanvas, "./canvas_random_rectangle.jpg");
    }
    ...
    public static int[][] stretchHorizontally(int[][] imageTwoD) {
        ...
        for (int i = 0; i < imageTwoD.length; i++){
            for (int j = 0; j < imageTwoD[0].length; j++){
                int iter = j*2;
                stretchedImage[i][iter] = imageTwoD[i][j];
                stretchedImage[i][iter+1] = imageTwoD[i][j];
            }
        }
    ...
    public static int[][] invertImage(int[][] imageTwoD) {
        ...
        for (int i = 0; i < imageTwoD.length; i++){
            for (int j = 0; j < imageTwoD[0].length; j++){
                invertedImage[i][j] = imageTwoD[imageTwoD.length-i-1][imageTwoD[i].length-j-1];
            }
        }
    ...
    public static int[][] paintRandomImage(int[][] canvas) {
        ...
        for (int i = 0; i < canvas.length; i++){
            for (int j = 0; j < canvas[0].length; j++){
                ...
                int[] rgbaArray = {redVal, greenVal, blueVal, 255};
                canvas[i][j] = getColorIntValFromRGBA(rgbaArray);
            }
        }
        return canvas;
    }
    ...
    public static int[][] generateRectangles(int[][] canvas, int numRectangles) {
        ...
        for (int num = 0; num<numRectangles; num++){
            ...
            int rowPosition = rand.nextInt(canvas.length - width);
            int colPosition = rand.nextInt(canvas[0].length - height);
